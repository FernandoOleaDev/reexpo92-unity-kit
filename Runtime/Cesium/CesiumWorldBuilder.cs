...
namespace ReExpo92.WorldKit.Cesium
{
    ...
    public class CesiumWorldBuilder : IWorldBuilder
    {
        ...
        public GameObject Build(WorldBuildOptions o)
        {
            ...
            var zoneRecords = new List<(Mesh mesh, Transform owner, List<CesiumGlobeAnchor> verts)>();
            ...
            if (o.ShowZones && o.Data?.Zones != null && o.Data.Zones.Count > 0)
            {
                ...
                foreach (var z in o.Data.Zones)
                {
                    ...
                    zoneRecords.Add((mesh, zgo.transform, verts));
                }
            }
        ...
        static async void AdjustHeightsAsync(
            ...
            List<(Mesh mesh, Transform owner, List<CesiumGlobeAnchor> verts)> zones)
        {
            ...
            // reconstruir los volúmenes de zona tras mover los vértices al terreno
        ...
        static void BuildPin(Transform parent, Material poleMat, Material headMat)
        {
            ...
            head.AddComponent<ReExpoBillboardBall>(); // mira a cámara + inclinación 45°
        }
